import { z } from "zod"
import { extractPages, formatExtract } from "./_kagi"

const tool = Object.assign((i: Record<string, unknown>) => i, { schema: z })

export default tool({
  description:
    "Extract clean markdown content from 1–10 URLs using Kagi's Extract API. " +
    "Returns rendered text content stripped of ads and navigation — ideal for reading articles, docs, and web pages.",
  args: {
    urls: tool.schema.array(tool.schema.string().url()).min(1).max(10).describe("Array of 1–10 HTTP(S) URLs to extract content from"),
    timeout: tool.schema.number().optional().describe("Time budget in seconds for the bulk extraction (out-of-range values are clamped by Kagi)"),
    max_chars: tool.schema.number().optional().describe("Maximum characters to return per URL (default: no limit)"),
  },
  async execute(args: { urls: string[]; timeout?: number; max_chars?: number }) {
    const result = await extractPages(args.urls, { timeout: args.timeout })
    if (!result.ok) return result.error
    const pages = result.data.data ?? []
    const trace = result.data.meta?.trace
    const output = formatExtract(pages, args.max_chars)
    return trace ? output + `\n\n---\n*Trace: \`${trace}\`*` : output
  },
})
